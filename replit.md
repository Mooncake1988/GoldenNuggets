...
- **Booking Integration**: Optional booking URL field for accommodation-type locations. Admins can paste an external booking link (Airbnb, Booking.com, Vrbo, etc.) and a "Book This Place" card appears on the location detail page with platform-aware button text. The section only displays when a booking URL is set.
